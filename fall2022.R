...
mcf <- readHumdrum('.*rap')
IfIRuledTheWorld <- readHumdrum('KurtisBlow_IfIRuledTheWorld.rap')

IfIRuledTheWorldDF <- as.data.frame(IfIRuledTheWorld)


threegramed <- within(IfIRuledTheWorld, paste(Token, Token[lag = -1:-2]), subset = Spine == 1, complement = Token)
...
capture.output(rhythm_list, file = "rhythm_IfIRuledTheWorld_threegramed.csv")
...
for (i in 1:nrow(IfIRuledTheWorldDF)){
  ipa_list <- append(ipa_list, IfIRuledTheWorldDF[i,6])
}

rhyme_list <- list()

for (i in 1:nrow(IfIRuledTheWorldDF)){
  rhyme_list <- append(rhyme_list, IfIRuledTheWorldDF[i,5])
}

stress_list <- list()

for (i in 1:nrow(IfIRuledTheWorldDF)){
  stress_list <- append(stress_list, IfIRuledTheWorldDF[i,2])
}

capture.output(ipa_list, file = "IPA_IfIRuledTheWorld.csv")

capture.output(rhyme_list, file = "rhyme_IfIRuledTheWorld.csv")

capture.output(stress_list, file = "stress_IfIRuledTheWorld.csv")
count <- 0
phrase_list <- list()
for (i in 1:nrow(IfIRuledTheWorldDF)){
  print(IfIRuledTheWorldDF[i,7])
  if(IfIRuledTheWorldDF[i,7] == "."){
    count <- count + 1
  }
...
capture.output(phrase_list, file = "phrase_IfIRuledTheWorld.csv")
