...
mcf <- readHumdrum('.*rap')
DooWop <- readHumdrum('LaurynHill_DooWopThatThing.rap')

DooWopDF <- as.data.frame(DooWop)


threegramed <- within(DooWop, paste(Token, Token[lag = -1:-15]), subset = Spine == 1, complement = Token)
...
capture.output(rhythm_list, file = "rhythm_DooWop_sixteengramed.csv")
...
for (i in 1:nrow(DooWopDF)){
  ipa_list <- append(ipa_list, DooWopDF[i,6])
}

rhyme_list <- list()

for (i in 1:nrow(DooWopDF)){
  rhyme_list <- append(rhyme_list, DooWopDF[i,5])
}

stress_list <- list()

for (i in 1:nrow(DooWopDF)){
  stress_list <- append(stress_list, DooWopDF[i,2])
}

capture.output(ipa_list, file = "IPA_DooWop.csv")

capture.output(rhyme_list, file = "rhyme_DooWop.csv")

capture.output(stress_list, file = "stress_DooWop.csv")
count <- 0
phrase_list <- list()
for (i in 1:nrow(DooWopDF)){
  print(DooWopDF[i,7])
...
capture.output(phrase_list, file = "phrase_DooWop.csv")
