...
mcf <- readHumdrum('.*rap')
CantTouchThis <- readHumdrum('MCHammer_UCantTouchThis.rap')

CantTouchThisDF <- as.data.frame(CantTouchThis)


threegramed <- within(CantTouchThis, paste(Token, Token[lag = -1:-2]), subset = Spine == 1, complement = Token)
...
capture.output(rhythm_list, file = "rhythm_CantTouchThis_threegramed.csv")
...
for (i in 1:nrow(CantTouchThisDF)){
  ipa_list <- append(ipa_list, CantTouchThisDF[i,6])
}

rhyme_list <- list()

for (i in 1:nrow(CantTouchThisDF)){
  rhyme_list <- append(rhyme_list, CantTouchThisDF[i,5])
}

stress_list <- list()

for (i in 1:nrow(CantTouchThisDF)){
  stress_list <- append(stress_list, CantTouchThisDF[i,2])
}

capture.output(ipa_list, file = "IPA_CantTouchThis.csv")

capture.output(rhyme_list, file = "rhyme_CantTouchThis.csv")

capture.output(stress_list, file = "stress_CantTouchThis.csv")
count <- 0
phrase_list <- list()
for (i in 1:nrow(CantTouchThisDF)){
  print(CantTouchThisDF[i,7])
  if(CantTouchThisDF[i,7] == "."){
    count <- count + 1
  }
...
capture.output(phrase_list, file = "phrase_CantTouchThis.csv")
