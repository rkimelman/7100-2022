...
mcf <- readHumdrum('.*rap')
GoodVibrations <- readHumdrum('MarkyMark_GoodVibrations.rap')

GoodVibrationsDF <- as.data.frame(GoodVibrations)


threegramed <- within(GoodVibrations, paste(Token, Token[lag = -1:-2]), subset = Spine == 1, complement = Token)
...
capture.output(rhythm_list, file = "rhythm_GoodVibrations_threegramed.csv")
...
for (i in 1:nrow(GoodVibrationsDF)){
  ipa_list <- append(ipa_list, GoodVibrationsDF[i,6])
}

rhyme_list <- list()

for (i in 1:nrow(GoodVibrationsDF)){
  rhyme_list <- append(rhyme_list, GoodVibrationsDF[i,5])
}

stress_list <- list()

for (i in 1:nrow(GoodVibrationsDF)){
  stress_list <- append(stress_list, GoodVibrationsDF[i,2])
}

capture.output(ipa_list, file = "IPA_GoodVibrations.csv")

capture.output(rhyme_list, file = "rhyme_GoodVibrations.csv")

capture.output(stress_list, file = "stress_GoodVibrations.csv")
count <- 0
phrase_list <- list()
for (i in 1:nrow(GoodVibrationsDF)){
  print(GoodVibrationsDF[i,7])
  if(GoodVibrationsDF[i,7] == "."){
    count <- count + 1
  }
...
capture.output(phrase_list, file = "phrase_GoodVibrations.csv")
