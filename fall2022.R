...
mcf <- readHumdrum('.*rap')
StandUp <- readHumdrum('Ludacris_StandUp.rap')

StandUpDF <- as.data.frame(StandUp)


threegramed <- within(StandUp, paste(Token, Token[lag = -1:-2]), subset = Spine == 1, complement = Token)
...
capture.output(rhythm_list, file = "rhythm_StandUp_threegramed.csv")
...
for (i in 1:nrow(StandUpDF)){
  ipa_list <- append(ipa_list, StandUpDF[i,6])
}

rhyme_list <- list()

for (i in 1:nrow(StandUpDF)){
  rhyme_list <- append(rhyme_list, StandUpDF[i,5])
}

stress_list <- list()

for (i in 1:nrow(StandUpDF)){
  stress_list <- append(stress_list, StandUpDF[i,2])
}

capture.output(ipa_list, file = "IPA_StandUp.csv")

capture.output(rhyme_list, file = "rhyme_StandUp.csv")

capture.output(stress_list, file = "stress_StandUp.csv")
count <- 0
phrase_list <- list()
for (i in 1:nrow(StandUpDF)){
  print(StandUpDF[i,7])
...
capture.output(phrase_list, file = "phrase_StandUp.csv")
